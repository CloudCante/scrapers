...
// Configurable output CSV file - using batch 8 output
const OUTPUT_CSV = process.argv[2] || 'output_batch8.csv';
...
(async () => {
  ...
  let batch = [];
  try {
    batch = JSON.parse(fs.readFileSync('serial_batch_8.json'));
    console.log(`Loaded ${batch.length} serials from batch 8 file`);
  } catch (e) {
    console.log('Could not read serial_batch_8.json, using test serial.');
    batch = [{
  ...
  if (unprocessedSerials.length === 0) {
    console.log('All serials in batch 8 have already been processed!');
    process.exit(0);
  }
  console.log(`Found ${unprocessedSerials.length} unprocessed serials out of ${batch.length} total in batch 8`);
  ...
  console.log('Starting automated processing of batch 8 with inherited cookies...');
  let successCount = 0;
  let failCount = 0;
