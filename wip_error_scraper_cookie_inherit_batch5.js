...
// Configurable output CSV file - using batch 5 output
const OUTPUT_CSV = process.argv[2] || 'output_batch5.csv';
...
(async () => {
  ...
  let batch = [];
  try {
    batch = JSON.parse(fs.readFileSync('serial_batch_5.json'));
    console.log(`Loaded ${batch.length} serials from batch 5 file`);
  } catch (e) {
    console.log('Could not read serial_batch_5.json, using test serial.');
    batch = [{
  ...
  if (unprocessedSerials.length === 0) {
    console.log('All serials in batch 5 have already been processed!');
    process.exit(0);
  }
  console.log(`Found ${unprocessedSerials.length} unprocessed serials out of ${batch.length} total in batch 5`);
  ...
  console.log('Starting automated processing of batch 5 with inherited cookies...');
  let successCount = 0;
  let failCount = 0;
